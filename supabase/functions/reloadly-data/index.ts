...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { operator_id, product_code, amount, recipient, recipient_phone, order_ref } = await req.json();
    ...
    const { error: deductErr } = await supabase.rpc("deduct_wallet", {
      ...
      p_amount: amount,
      p_description: `Data bundle - ${product_code} to ${recipient}`,
    ...
    if (!providerSuccess) {
      await supabase.rpc("refund_wallet", {
        p_user_id: userData.user.id,
        p_amount: amount,
        p_description: `Refund for failed data purchase`,
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Data bundle delivered to ${recipient}`,
        order_id: order?.id,
        provider_ref: providerRef,
      }),
