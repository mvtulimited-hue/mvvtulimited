...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { product_id, amount, unit_price, recipient_email, order_ref } = await req.json();
    ...
    const totalCost = amount * unit_price;

    const { error: deductErr } = await supabase.rpc("deduct_wallet", {
      p_user_id: userData.user.id,
      p_amount: totalCost,
      p_description: `Gift card purchase - ${product_id} x${amount}`,
    ...
    try {
      ...
      const orderResp = await fetch("https://giftcards.reloadly.com/orders", {
        method: "POST",
        headers: {
      ...
      providerResponse = await orderResp.json();

      if (orderResp.ok && providerResponse.transactionId) {
    ...
    if (!providerSuccess) {
      await supabase.rpc("refund_wallet", {
        p_user_id: userData.user.id,
        p_amount: totalCost,
        p_description: `Refund for failed gift card purchase`,
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "Gift card purchase failed. Your wallet has been refunded.",
          order_id: order?.id,
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Gift card sent to ${recipient_email}`,
        order_id: order?.id,
        provider_ref: providerRef,
        card_details: providerResponse,
      }),
